...
│    - Upsamples frame rate from 100 Hz \to 24,000 Hz \in <8ms             │
└────────────────────────────────────────────────────────────────────────┘
                │
                ▼
┌────────────────────────────────────────────────────────────────────────┐
...
Conversations where response \times remained within the optimal **200ms to 350ms window** are rewarded ($\mathbf{y}_w$), training the neural network to avoid long pauses or rushed verbal delivery.
...
Cascaded systems chain three separate API services (STT $\r\r\rightarrow$ LLM $\r\r\rightarrow$ TTS). Each stage must serialize its output into intermediate text formats and make network calls, accumulating **650ms to 1,400ms of lag**.
...
Optimized cascaded stacks cost **\$0.084 \to \$0.140 per minute** across fragmented third-party invoices. Tough Tongue AI provides an all-inclusive platform with carrier SIP trunking for a flat rate of **₹3.50 per minute** (**\$0.042/min**).
