...
2. 1962: IBM "Shoebox" (Seattle World's Fair)
   ...
   - Action: Instructed an electric adding machine to print arithmetic calculations.
```
...
1980s: Energy Threshold Gating: E = \sum x^2(n) > E_{\text{threshold}} (Sensitive to noise)
...
1. 1986: IBM Tangora (Thomas J. Watson Research Center)
   ...
   - Requirement: Isolated speech (Users had to pause 250ms between every spoken word).
...
2. 1990: Dragon Dictate (Dragon Systems by Jim and Janet Baker)
   ...
   - Limitation: Required 45 minutes of user voice enrollment training to adapt acoustic models.
...
2. 1991: Wall Street Journal (WSJ) Corpus (NIST Benchmark)
   - Expanded vocabulary from 1,000 words (Resource Management) to 20,000 words.
...
│    - Finds most probable word sequence: \hat{W} = \arg\max P(O|W) P(W) │
└────────────────────────────────────────────────────────────────────────┘
```
...
1. Connectionist Temporal Classification (CTC Loss):
   - Eliminated complex HMM alignment lattices, training neural networks directly on audio-text pairs.
...
│ - Linear sequence recurrence: h_t = ar{A} h_{t-1} + ar{B} x_t      │
└────────────────────────────────────────────────────────────────────────┘
              │
              ▼
...
- Eliminates intermediate text conversions.
- Response Latency drops from 1,200ms to <180ms.
...
2. 2006 - 2018: Directed Dialogue Speech IVR (Nuance / Uniphore)
   ...
   - High caller drop-off due to regional dialect misrecognition.

3. 2019 - 2023: Cascaded Conversational Chatbots with Voice Wrappers
   - Transcribed telephony audio to text, passed to chat engines.
...
class SpeechRecognitionHistoricalSimulator:
    ...
    from 1952 analog filter thresholds to 1980s statistical HMMs to 2026 Native V2V models.
    """
    def __init__(self):
