...
>   3. **2020 to 2023 (The Cascaded Voicebot Era):** Modular pipelines (STT $\rightarrow$ LLM $\rightarrow$ TTS) chaining independent APIs with **800ms to 1,500ms latency**.
...
The discrete recurrence $\mathbf{h}_t = ar{\mathbf{A}} \mathbf{h}_{t-1} + ar{\mathbf{B}} x_t$ emits audio chunks in **&lt;40ms Time-to-First-Audio (TTFA)**, allowing modern synthesizers to generate broadcast-grade 24kHz audio in real time.
...
- Structural Failure: Strips vocal pitch (F0), laughter, and emotion at text boundaries.
```

While superior to touch-tone IVR, cascaded voicebots suffered from compounding latency, fragmented multi-vendor API bills (**\$0.084 to \$0.140/min**), and frequent conversational collisions.
...
2. PagedAttention Virtual Memory Manager:
   ...
   - Reduces GPU memory waste from 65% to under 4%, driving costs down to ₹3.50/min.
```
...
In 1996 touch-tone IVR, navigating a billing menu required $N_{\text{turns}} = 6$ with $T_{\text{task}} = 180\text{ seconds}$, yielding $R_{\text{info}} \approx 0.05\text{ bits/sec}$.
...
- 1995 (Enterprise Touch-Tone IVR):   ~$0.45 / Automated Call ($5.50 when escalated to human)
...
Human contact center interactions cost **\$5.50 to \$12.00 per completed call**. Tough Tongue AI handles the same conversation for **₹3.50 per minute** (**\$0.042/min**), delivering over **75% operational cost savings**.
