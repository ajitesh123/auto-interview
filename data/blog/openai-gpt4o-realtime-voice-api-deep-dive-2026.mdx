...
The Realtime API relies entirely on stateful WebSocket connections. It does not support standard stateless HTTP requests. You begin by sending a `session.update` payload upon connection.

This payload configures voices, tool definitions, and turn detection parameters. You then continuously stream chunks of base64-encoded PCM audio. These chunks are sent via `input_audio_buffer.append` events.

The server handles Voice Activity Detection internally by default. When it detects the end of user speech, it fires a `response.create` event automatically. The model then streams back `response.audio.delta` packets containing synthesized audio.

Mid-utterance interruption is handled elegantly by the protocol state machine. If the user speaks while the model is responding, the server emits an `input_audio_buffer.speech_started` event. You must immediately send a `response.cancel` message to stop the current generation.

This bidirectional signaling is crucial for natural conversations. It prevents the model from talking over the user. It also saves money by halting token generation immediately.

The session lifecycle requires careful state tracking on the client side. You must maintain the current response ID and item ID. This ensures you cancel the correct response during interruptions.
