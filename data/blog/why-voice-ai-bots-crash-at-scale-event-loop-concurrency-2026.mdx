...
• Forwarding RTP audio frame               • Resampling 24kHz audio to 8kHz PCM
• Pushing LLM token to WebSocket           • Parsing 50KB JSON CRM schema
...
# Bind to internal port — reverse proxy via Nginx/Envoy in production
bind = "0.0.0.0:8080"
    ...
    def report_carrier_response(self, sip_code: int):
        ...
        if sip_code == 503 or sip_code == 429:  # Service Unavailable / Too Many
            ...
            if self._consecutive_503s >= 3:
                ...
                print(f"[SIP] Carrier overloaded — throttling to {self.max_cps} CPS")
