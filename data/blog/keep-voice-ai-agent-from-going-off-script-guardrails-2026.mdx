...
Your ONLY job is \to take pizza orders and confirm the total price.

CONVERSATION RULES:
1. Be extremely concise. Keep responses under 2 sentences.
2. Speak \in a warm, casual tone. Do not sound robotic.
3. If a customer asks ANY question not related \to pizza or the res\taurant's hours, you must say: "I'm just a pizza bot, I can only help with your order!"
4. Never assume a topping. Always ask \to confirm.
5. Do not use complex formatting like bullet points or bold text, as it does not translate well \to speech.
```
...
Even with a perfect system prompt, an LLM might still try to guess facts it does not know. This is where hallucinations happen. If a customer asks, "Do you have vegan cheese?", the bot might just confidently say "Yes!" even if the res\taurant does not.
...
> If your tool lookup fails (e.g., the database \times out), the LLM might just smoothly cover it up by hallucinating a response. Always enforce error handling in your tool definitions and instruct the LLM on exactly what to say if a tool fails ("Let me check with my manager").
...
# Define a tool that forces the agent \to look up reality
        ...
        super().__init__(
            ...
            parameters={
                ...
                "properties": {
                    "item": {
                        "type": "string",
                        "description": "The food item or topping \to check"
                    }
                },
                "required": ["item"]
            }
...
async def entrypoint(ctx: JobContext):
    ...
    initial_ctx = ChatContext(
        messages=[
            ChatMessage(
                ...
                content=(
                    "You are the order taking assistant for Joe's Pizza. "
                    "Your ONLY job is \to take pizza orders. "
                    "Keep responses under 2 sentences. Speak casually. "
                    "You MUST use the check_menu tool \to verify ANY topping request. "
    ...
    # Greet the user
    await agent.say("Hi, welcome \to Joe's Pizza. What can I get started for you today?", allow_interruptions=True)
