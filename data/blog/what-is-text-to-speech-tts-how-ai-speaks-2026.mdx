...
>
> - **What is Text-to-Speech (TTS)?** Text-to-Speech is the computational synthesis of natural, intelligible human spoken audio waveforms directly from written alphanumeric text tokens.
...
Converting written alphanumeric text into physical soundwaves requires navigating two complex transformations: **linguistic text normalization** and **acoustic waveform synthesis**.
...
│    - Converts text into International Phonetic Alphabet (IPA) phonemes │
...
└────────────────────────────────────────────────────────────────────────┘
                                   │
                                   ▼
[Synthesized Audio Stream Delivered to Caller via Carrier SIP Trunk]
```
...
$$H(z) = rac{1}{1 - \sum_{k=1}^{P} a_k z^{-k}}$$
...
$$ar{\mathbf{A}}_t = \exp(\Delta_t \mathbf{A}), \quad ar{\mathbf{B}}_t = (\Delta_t \mathbf{A})^{-1}(\exp(\Delta_t \mathbf{A}) - \mathbf{I}) \cdot (\Delta_t \mathbf{B}_t)$$

The discrete sequence recurrence is formulated as:

$$\mathbf{h}_t = ar{\mathbf{A}}_t \mathbf{h}_{t-1} + ar{\mathbf{B}}_t x_t, \quad y_t = \mathbf{C}_t \mathbf{h}_t + \mathbf{D} x_t$$
...
│    - Increases temporal sampling rate from 100 Hz to 24,000 Hz         │
└────────────────────────────────────────────────────────────────────────┘
                     │
                     ▼
┌────────────────────────────────────────────────────────────────────────┐
...
$$
\mathcal{L}_{	ext{FM}}(G; D) = \mathbb{E}_{(\mathbf{x}, \mathbf{s})} \left[ \sum_{i=1}^{T} rac{1}{N_i} \| D^{(i)}(\mathbf{x}) - D^{(i)}(G(\mathbf{s})) \|_1
ight]
$$
...
$$\hat{F}_0(t) = \mu_{F_0} + \sigma_{F_0} \cdot 	anh(	ext{AcousticPredictor}(\mathbf{h}_t))$$
...
│ - Transcribes synthetic audio to measure Synthetic WER                 │
└────────────────────────────────────────────────────────────────────────┘
               │
               ▼
┌────────────────────────────────────────────────────────────────────────┐
...
$$ ext{WER}\_{ ext{synth}} = rac{S + D + I}{N}$$
...
[AI Voice Agent Speaking Audio Output via Phone Line]
                          │
                          ▼
[User Speaks Mid-Sentence]: "Wait, can we reschedule to Friday?"
                          │
                          ▼
┌────────────────────────────────────────────────────────────────────────┐
...
class StreamingTTSClient:
    ...
    async def stream_text_to_speech(self, text_token_generator: AsyncGenerator[str, None]):
        ...
        async with websockets.connect(self.websocket_uri, extra_headers=headers) as ws:
            ...
            async def send_text_tokens():
                async for token in text_token_generator:
                    # Stream individual generated words to voice vocoder
                    await ws.send(json.dumps({"type": "text_chunk", "text": token}))
            ...
            async def receive_audio_chunks():
                ...
                async for message in ws:
                    if isinstance(message, bytes):
                        ...
                        # Playback 20ms linear PCM audio chunk back to user
...
Text normalization and Inverse Text Normalization (ITN) rules expand abbreviations (_"Dr."_ $\rightarrow$ _"Doctor"_), dates (_"Jan 12"_ $\rightarrow$ _"January twelfth"_), and currency (_"\$50"_ $\rightarrow$ _"fifty dollars"_) before phonetic synthesis.
...
Commercial TTS APIs charge between **\$0.05 and \$0.30 per 1,000 characters** (approx. **\$0.09 to \$0.21 per minute**). Tough Tongue AI bundles speech synthesis, language reasoning, speech recognition, and telephony for an all-inclusive flat rate of **₹3.50 per minute** (**\$0.042/min**).
