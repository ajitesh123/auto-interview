...
## The Architecture: How Gemini 2.0 Flash Live Works

Traditional voice AI systems use a cascade pipeline architecture. This involves sending audio to a speech-to-text model, processing the text through an LLM, and generating speech with a text-to-speech model. Each step adds sequential latency that makes conversations feel robotic and slow.

Gemini 2.0 Flash Live replaces this pipeline with a single native multimodal model. The model ingests raw audio directly as tokens and outputs audio tokens in the same forward pass. This native audio tokenization preserves prosody, emotion, and conversational cadence perfectly.

The entire communication layer runs over a bidirectional WebSocket session. Developers initiate a connection via the Google AI SDK and maintain an open channel. This persistent connection allows continuous streaming of raw PCM audio chunks without HTTP request overhead.
