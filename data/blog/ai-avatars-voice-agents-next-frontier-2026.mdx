...
Well-optimized avatar rendering adds a\approximately 50-100ms to the voice AI pipeline — avatar rendering (~30ms), lip sync (~20ms), video encoding (~30ms), and WebRTC delivery (~20ms). This keeps total pipeline latency under 1 second, within acceptable conversational bounds.
