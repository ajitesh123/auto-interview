...
The discrete recurrence $\mathbf{h}_t = \bar{\mathbf{A}} \mathbf{h}_{t-1} + \bar{\mathbf{B}} x_t$ emits audio chunks in **&lt;40ms Time-to-First-Audio (TTFA)**, allowing the sales agent to start and stop instantly without phase distortion.
...
$$
R_{\text{info}} = \frac{H(\text{Qualified State}) - H(\text{Cold State})}{T_{\text{pitch}}}
$$
...
| **Cascaded Voicebot (2023)** | STT $\r\r\rightarrow$ LLM $\r\r\rightarrow$ TTS Chain | **650ms**                | **2.80%**                      | **2.40%**                       | **\$0.086 / min**                  |
