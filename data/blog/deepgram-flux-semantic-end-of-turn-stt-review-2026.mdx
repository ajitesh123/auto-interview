...
## How Deepgram Flux Works: Acoustic and Linguistic Joint Modeling

Deepgram Flux solves the interruption problem by combining acoustic analysis with large language model linguistic prediction. The underlying model predicts sentence boundary probability directly from audio frames and partial transcript semantics. It does not just wait for quiet audio frames.

As the audio streams into the Deepgram WebSocket, the system generates partial transcripts. The Flux model analyzes these words in real time. It evaluates if the sentence is grammatically complete.

If the semantic completion probability is high, it triggers a turn completion event rapidly. If the transcript is incomplete, the model holds the turn open even during long silences. This allows users to pause naturally without being interrupted.

This joint modeling is exposed through the `speech_final` versus `is_final` event stream architecture. The `is_final` event indicates a hard acoustic boundary. The `speech_final` event indicates semantic completion based on the linguistic model.
